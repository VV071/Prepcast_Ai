...
import { motion, useMotionValue, useSpring, useTransform, AnimatePresence, transform } from "framer-motion";
...
function IconContainer({ mouseX, title, icon, href }) {
    ...
    const widthTransform = useTransform(distance, (val) => {
        if (val === Infinity || isNaN(val)) return 40;
        return transform(val, [-150, 0, 150], [40, 80, 40]);
    });
    ...
    const widthIconTransform = useTransform(distance, (val) => {
        if (val === Infinity || isNaN(val)) return 20;
        return transform(val, [-150, 0, 150], [20, 40, 20]);
    });
    const heightIconTransform = useTransform(distance, (val) => {
        if (val === Infinity || isNaN(val)) return 20;
        return transform(val, [-150, 0, 150], [20, 40, 20]);
    });
